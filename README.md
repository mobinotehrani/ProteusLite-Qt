...
نسخه موجود در شاخه `develop` شامل بخش‌های پیاده‌سازی‌شده تا پایان بخش هشتم است. در این نسخه کاربر می‌تواند پروژه جدید بسازد، قطعات را از کتابخانه انتخاب کند، آن‌ها را روی بوم قرار دهد، مشخصاتشان را تغییر دهد، بین پایه‌ها سیم بکشد و نتیجه ساده شبیه‌سازی را مشاهده کند.
...
- نمایش وضعیت مدار در پنل حرفه‌ای `Circuit Monitor`
...
- نمایش وضعیت قطعات پیشرفته در پنل `Advanced Devices`
- کنترل Run، Pause، Stop و Step برای موتور شبیه‌سازی
- حل گره‌ای مسیرهای مقاومتی و محاسبه جریان مدارهای سری شامل LED
- رنگ زنده سیم‌ها بر اساس HIGH، LOW، مقدار آنالوگ یا خطای شبکه
- تعامل زنده با Switch، Push Button، Keypad و Potentiometer
- اجرای دستوربه‌دستور MCU در حالت Step
...
نسخه‌های قبلی پروژه با Qt 6 و MinGW Build و اجرا شده‌اند. نسخه فعلی پس از اضافه شدن کنترل‌های بخش هشتم باید یک‌بار به‌صورت تمیز در Qt Creator Build شود و سناریوهای `SECTION08_README_FA.md` روی آن اجرا شوند.
...
هر بخش مسئولیت مشخصی دارد. برای نمونه، `CanvasView` رفتار بوم را کنترل می‌کند، `ComponentLibraryPanel` کتابخانه قطعات را می‌سازد، `CircuitGraph` اطلاعات اتصال‌ها را نگه می‌دارد و کنترلرهای بخش پنجم تا هشتم رابط بین مدل مدار و محیط گرافیکی هستند.
